...
open System
    ...
    type OptimalDiscountFactors(p1: float, p2: float, p3: float, p4: float, p5: float,
                                 p6: float, p7: float, p8: float, p9: float, p10: float,
                                 p12: float, p15: float, p20: float) =
        inherit DownhillSimplex(Vertex [1.0; 1.0; 1.0; 1.0; 0.9; 0.9; 0.9; 0.9; 0.9; 0.9;
        ...
        let par' = [99.9; p1; p2; p3; p4; p5; p6; p7; p8; p9; p10; 99.9; p12; 99.9; 99.9; p15; 99.9; 99.9; 99.9; 99.9; p20]
        ...
        let zeroCouponRate (dur: int) =
            DF'.[dur] ** (-1.0 / (float dur)) - 1.0

        let forwardRate (dur: int) =
            if dur <= T2 then DF'.[dur - 1] / DF'.[dur] - 1.0
                         else UFR
        ...
        member this.fiRate (dur: int) =
            let rec fi dur =
                match dur with
                | 0 | 1 -> zeroCouponRate 1
                | dur -> ((1.0 + fi (dur - 1)) ** (float dur - 1.0) * (1.0 + weightedForwardRate dur)) ** (1.0 / float dur) - 1.0
            fi dur
        ...
        override this.objective (Vertex v) =
            ...
            if not (List.forall (fun df -> df > 0.0) DF.[1 .. 20]) then 100.0 else
